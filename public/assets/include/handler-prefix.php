...
if (!defined('ESP-FIRST-INCLUDED')) { echo "In order to conduct surveys, please include phpESP.first.php (not handler-prefix.php)"; exit; }
...
if(isset($_REQUEST['results']) || isset($_REQUEST['results'])) { $GLOBALS['errmsg'] = mkerror(_('Error processing survey: Security violation.')); return; }
...
if ($ESPCONFIG['auth_response']) {
    ...
    if ($GLOBALS['ESPCONFIG']['auth_mode'] == 'basic') {
      $espuser = ''; $esppass = '';
      if (isset($_SERVER['PHP_AUTH_USER'])) { $espuser = $_SERVER['PHP_AUTH_USER']; }
      if (isset($_SERVER['PHP_AUTH_PW'])) { $esppass = $_SERVER['PHP_AUTH_PW']; }
    }
    elseif ($GLOBALS['ESPCONFIG']['auth_mode'] == 'form') {
      if (isset($_REQUEST['username']) && ($_REQUEST['username'] != "")) { $_SESSION['espuser'] = $_REQUEST['username']; }
      if (isset($_SESSION['espuser'])) { $espuser = $_SESSION['espuser']; }
      else { $espuser = ""; }

      if (isset($_REQUEST['password']) && ($_REQUEST['password'] != "")) { $_SESSION['esppass'] = $_REQUEST['password']; }
      if (isset($_SESSION['esppass'])) { $esppass = $_SESSION['esppass']; }
      else { $esppass = ""; }
    }

    if(!survey_auth($sid, $espuser, _addslashes($esppass), $esppass, $_css, $_title)) { return; }
