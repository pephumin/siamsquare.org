...
class dBug {
	...
	function dBug($var, $forceType = "", $bCollapsed = false) {
		if (!defined('BDBUGINIT')) { define("BDBUGINIT", TRUE); $this->initJSandCSS(); }
		$arrAccept = array("array", "object", "xml");
		$this->bCollapsed = $bCollapsed;
		if (in_array($forceType, $arrAccept)) { $this->{"varIs".ucfirst($forceType)}($var); }
	...
	function makeTableHeader($type, $header, $colspan = 2) {
		...
		echo "<table class=\"table table-condensed dBug_".$type."\">";
		// echo "  <tr>";
	...
	function makeTDHeader($type, $header) {
		...
		echo "  <tr".$str_d.">";
		echo "    <td width=\"10%\" onClick='dBug_toggleRow(this)' class=\"dBug_".$type."Key\">".$header."</td>";
		echo "    <td>";
	}
	...
	function checkType($var) {
		switch(gettype($var)) {
			case "resource": $this->varIsResource($var); break;
			case "object": $this->varIsObject($var); break;
			case "array": $this->varIsArray($var); break;
			case "NULL": $this->varIsNULL(); break;
			case "boolean": $this->varIsBoolean($var); break;
			default: $var=($var=="") ? "[empty string]" : $var; echo "<table class=\"table table-condensed\"><tr>\n<td>".$var."</td>\n</tr>\n</table>\n"; break;
		}
	}
	...
	function varIsArray($var) {
		...
		if (is_array($var)) {
			foreach ($var as $key => $value) {
				...
				else { $value = (trim($value) == "") ? "[empty string]" : $value; echo $value; }
				echo $this->closeTDRow();
			}
		}
		else { echo "<tr><td style=\"padding:0px\">".$this->error("array").$this->closeTDRow(); }
		...
		if (is_object($var)) {
			...
			foreach ($arrObjVars as $key=>$value) {
				...
				if (is_object($value)||is_array($value)) {
					$var_ser = serialize($value);
					if (in_array($var_ser, $this->arrHistory, TRUE)) { $value = (is_object($value)) ? "*RECURSION* -> $".get_class($value) : "*RECURSION*"; }
				}
				if (in_array(gettype($value), $this->arrType)) { $this->checkType($value); }
				else { echo $value; }
				echo $this->closeTDRow();
	...
	function varIsXmlResource($var) {
		...
		if ($bFile) { while ($data=str_replace("\n", "",fread($fp, 4096))) { $this->xmlParse($xml_parser, $data,feof($fp)); } }
		else {
			if (!is_string($var)) { echo $this->error("xml").$this->closeTDRow()."</table>\n"; return; }
			$data = $var;
		...
		echo <<<SCRIPTS
				...
				function dBug_toggleTable(source) {
					...
					if (document.all) {
						...
						for (var i=1;i<table.childNodes.length;i++) {
							...
							if (target.style) { dBug_toggleTarget(target,switchToState); }
						}
					}
				}
			</script>
			...
			<style type="text/css">
				...
				table.dBug_array, table.dBug_object, table.dBug_resource, table.dBug_resourceC, table.dBug_xml
					{ font-family:Verdana, Arial, Helvetica, sans-serif; color:#000000; font-size:10px; border-spacing:1px; border-collapse:separate; }
