...
class JSON2CSVutil {
  ...
  private function prependColumnNames() {
    foreach (array_keys($this->dataArray[0]) as $key) { $keys[0][$key] = $key; }
  ...
  function save2CSV($file) {
    if ($this->isItNested() || !is_array($this->dataArray)) {
      ...
      $fileIO = fopen($file, 'w+');
      foreach ($this->dataArray as $fields) { fputcsv($fileIO, $fields); }
      fclose($fileIO);
    }
  }
  ...
  function browserDL($CSVname) {
    if ($this->isItNested() || !is_array($this->dataArray)) {
      ...
      foreach ($this->dataArray as $fields) { fputcsv($output, $fields); }
    }
  }
